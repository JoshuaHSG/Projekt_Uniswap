...
@st.cache_data  
def load_data():
    return pd.read_csv('Mappe7.csv', delimiter=';')
...
# This is our filter Function for our Selection filters
def filter_data(data):
    ...
    if academic_level_filter != 'All':
        ...
        data = data[(data['Degree Level'] == academic_level_filter) | 
                    (data['Degree Level'] == 'Bachelor/Master')]
    if exchange_type_filter != 'All':

        data = data[(data['Exchange Program'] == exchange_type_filter) | 
                    (data['Exchange Program'] == 'Partner University/CEMS')]
    return data

# With the filter function we now also set up our Sidebars
with st.sidebar:
    # Filter options
    continent_filter = st.selectbox("Select Continent", ["All"] + sorted(data["Continent"].unique()))
    country_filter = st.selectbox("Select Country", ["All"] + sorted(data["Country"].unique()))
    academic_level_filter = st.selectbox("Select Degree Level", ["All", "Bachelor", "Master"])
    exchange_type_filter = st.selectbox("Select Exchange Program", ["All", "Partner University", "Freemover", "CEMS"])
    search_query = st.text_input("Search for a university")

    # Filter data based on the selection
    filtered_data = filter_data(data)

    # Create a button for each university in the sidebar
    for index, university in filtered_data.iterrows():
        if st.button(university['Name of University']):
...
if st.session_state.selected_university:
    ...
    st.title("Everything you need to know about doing an Exchange!")
